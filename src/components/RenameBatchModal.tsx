...
interface RenameBatchModalProps {
  ...
  isRenaming: boolean;
  handleRenameBatch: () => Promise<boolean | void>;
}
  ...
  return (
    <div className="fixed inset-0 bg-black/40 backdrop-blur-md z-50 flex items-center justify-center p-4">
      <div className="glass-panel rounded-[2rem] w-full max-w-md overflow-hidden shadow-2xl">
        ...
        <div className="p-8">
          <div className="mb-6">
            <label className="block text-[13px] font-semibold text-[var(--color-ios-text-secondary)] uppercase tracking-wider mb-2">
              {t.newBatchName}
            </label>
            <input
              type="text"
            ...
            <button
              onClick={async () => {
                ...
                if (!isValidBatchDate(finalName)) {
                  toast.error(t.invalidBatchDate);
                  return;
                }
                const success = await handleRenameBatch();
                if (success) {
              ...
              disabled={
                isRenaming ||
                !newBatchName.trim() ||
                padBatchId(newBatchName) === editingBatch.batchId
              }
